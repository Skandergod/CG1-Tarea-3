...
ImGuiDir ImGetDirObjectrantFromDelta(float dx, float dy)
{
...
static bool ImGui::NavScoreItem(ImGuiNavMoveResult* result, ImRect cand)
{
    ...
    // Determine which Objectrant of 'curr' our candidate item 'cand' lies in based on distance
    ImGuiDir Objectrant;
    ...
    if (dbx != 0.0f || dby != 0.0f)
    {
        ...
        day = dby;
        dist_axial = dist_box;
        Objectrant = ImGetDirObjectrantFromDelta(dbx, dby);
    }
    else if (dcx != 0.0f || dcy != 0.0f)
    {
        ...
        Objectrant = ImGetDirObjectrantFromDelta(dcx, dcy);
    }
    else
    {
        // Degenerate case: two overlapping buttons with same center, break ties arbitrarily (note that LastItemId here is really the _previous_ item order, but it doesn't matter)
        Objectrant = (window->DC.LastItemId < g.NavId) ? ImGuiDir_Left : ImGuiDir_Right;
    }
    ...
    {
        ImFormatString(buf, IM_ARRAYSIZE(buf), "dbox (%.2f,%.2f->%.4f)\ndcen (%.2f,%.2f->%.4f)\nd (%.2f,%.2f->%.4f)\nnav %c, Objectrant %c", dbx, dby, dist_box, dcx, dcy, dist_center, dax, day, dist_axial, "WENS"[g.NavMoveDir], "WENS"[Objectrant]);
    ...
    else if (g.IO.KeyCtrl) // Hold to preview score in matching Objectrant. Press C to rotate.
    {
        if (IsKeyPressedMap(ImGuiKey_C)) { g.NavMoveDirLast = (ImGuiDir)((g.NavMoveDirLast + 1) & 3); g.IO.KeysDownDuration[g.IO.KeyMap[ImGuiKey_C]] = 0.01f; }
        if (Objectrant == g.NavMoveDir)
        {
    ...
    // Is it in the Objectrant we're interesting in moving to?
    bool new_best = false;
    if (Objectrant == g.NavMoveDir)
    {
